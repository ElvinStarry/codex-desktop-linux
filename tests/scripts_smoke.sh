...
assert_contains "$pkg_root/DEBIAN/postinst" "codex_cleanup_user_service stop"
assert_contains "$pkg_root/DEBIAN/postinst" "codex_cleanup_user_service disable"
assert_not_contains "$pkg_root/DEBIAN/postinst" "codex_ensure_user_service_running"
assert_not_contains "$pkg_root/DEBIAN/postinst" "codex_start_enabled_user_service"
...
assert_not_contains "$pkg_root/opt/codex-desktop/.codex-linux/codex-packaged-runtime.sh" "is-enabled codex-update-manager.service"
...
test_fedora_dependency_bootstrap_installs_rpmbuild() {
    ...
    grep -Fq 'if [ "${CODEX_INSTALL_DEPS_SOURCE_ONLY:-0}" = "1" ]; then' "$install_deps" \
        || fail "Expected install-deps source-only guard in $install_deps"
...
assert_not_contains "$REPO_DIR/packaging/linux/codex-packaged-runtime.sh" "is-enabled codex-update-manager.service"
assert_not_contains "$REPO_DIR/packaging/linux/codex-packaged-runtime.sh" "codex-update-manager-launch-check"
assert_not_contains "$REPO_DIR/packaging/linux/codex-packaged-runtime.sh" "codex-update-manager check-now --if-stale"
...
assert_not_contains "$REPO_DIR/packaging/linux/codex-update-manager.postinst" "codex_start_enabled_user_service"
assert_not_contains "$REPO_DIR/packaging/linux/codex-desktop.install" "codex_start_enabled_user_service"
assert_not_contains "$REPO_DIR/packaging/linux/codex-desktop.spec" "codex_start_enabled_user_service"
assert_contains "$REPO_DIR/packaging/linux/codex-update-manager.postinst" "codex_cleanup_user_service disable"
assert_contains "$REPO_DIR/packaging/linux/codex-desktop.install" "codex_cleanup_user_service disable"
assert_contains "$REPO_DIR/packaging/linux/codex-desktop.spec" "codex_cleanup_user_service disable"
